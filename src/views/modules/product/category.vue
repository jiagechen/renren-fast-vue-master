...
export default {
  data() {
    return {
      ...
      maxLevel:0, //最大层级，递归函数countNodeLevel使用的，仅仅保存数据而已，无特殊意义
      dialogType:false, // true 添加  false 更新
      dialogVisible: false, //添加/修改的对话框
      categoryForm:{name:null,icon:null,productUnit:null,showStatus:1,sort:0,catId:null,catLevel:1},//修改或增加用的表单
      data: [], //接收的后台数据，用于展示树形结构的数据
      expandKeys: [], //默认展开的结点
      defaultProps: {
        children: "childrens", //指定结点的某个属性表示子结点
        label: "name", //指定结点的某个属性表示本结点
      },
    };
  },
  methods: {
    getCategory() { //加载树形结构所需数据
      this.$http({
    ...
    allowDrop(draggingNode, dropNode, type){
      ...
      let deep = Math.abs(this.maxLevel - draggingNode.level ) + 1;//从当前结点到最深结点共有几层
      if(type == 'inner'){
    ...
    },submitForm(){ //提交表单，根据dialogType字段判断是新增   还是   更新
